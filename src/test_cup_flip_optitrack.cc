...
//             tree.get_num_positions());
  ...
  // "Prep" postures.
  // place the gripper above the tray
...
//    }
    ...
    // Call ik to move the first mug to staging area.
    {
      ...
      // MAKE A cup_pose HERE (in the robot base frame)
      // cup frame is defined at bottom of cup, x points towards the handle, z points up
...
//    pcl::io::savePCDFileASCII("stage_scene.pcd", *scene);
    ...
    // in parallel.
    // this is retracting arm and put high above the staging area
    robot_comm.MoveJointDegrees(stage_prep_q_deg, 1.5, true); // <-- the 1.5 is a time param, might want to set these appropriately for slow movement
